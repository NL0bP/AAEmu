    ...
    public class GradeEnchant : SpecialEffectAction
    {
            ...
            int value4)
        {
            ...
            if (useCharm)
            {
                ...
                if (charmInfo.RequireGradeMin != -1 && item.Grade < (ItemGrade)charmInfo.RequireGradeMin)
                {
                    character.SendErrorMessage(Error.ErrorMessageType.NotEnoughRequiredItem);
                    return;
                }

                if (charmInfo.RequireGradeMax != -1 && item.Grade > (ItemGrade)charmInfo.RequireGradeMax)
                {
            ...
            if (item.Grade >= ItemGrade.Divine && (result == GradeEnchantResult.Success || result == GradeEnchantResult.GreatSuccess))
            {
        ...
        private int GoldCost(GradeTemplate gradeTemplate, Item item, int ItemType)
        {
            ...
            var itemGrade = gradeTemplate.Grade; //EnchantCost; TODO
