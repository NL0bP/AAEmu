   ...
   public class Skill
    {
        ...
        public void Use(Unit caster, SkillCaster casterCaster, SkillCastTarget targetCaster, SkillObject skillObject = null)
        {
            ...
            if (Template.DefaultGcd)
            {
                ...
                caster.GlobalCooldown = DateTime.UtcNow.AddMilliseconds(1000);
            }
            else
            {
                //TODO Apply Attack Speed * GCD
                caster.GlobalCooldown = DateTime.UtcNow.AddMilliseconds(Template.CustomGcd);
            }
        ...
        public void Apply(Unit caster, SkillCaster casterCaster, BaseUnit targetSelf, SkillCastTarget targetCaster, SkillObject skillObject)
        {
            ...
            foreach (var effect in Template.Effects)
            {
                foreach (var target in targets)
                {
                    ...
                    effect.Template?.Apply(caster, casterCaster, target, targetCaster, new CastSkill(Template.Id, TlId), this, skillObject, DateTime.UtcNow, packets);
                }
            }
            // Quick Hack
