...
namespace AAEmu.Game.Models.Game.Units
{
    public sealed class Mate : Unit
    {
        ...
        public uint Att1 { get; set; } // objId
        public AttachUnitReason Reason1 { get; set; }
        public uint Att2 { get; set; } // objId
        ...
        public override int MaxHp
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MaxHealth);
        ...
        public override int HpRegen
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.HealthRegen);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int PersistentHpRegen
        {
            get
            {
                var formula = FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate,
        ...
        public override int MaxMp
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MaxMana);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int MpRegen
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.ManaRegen);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override float LevelDps
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.LevelDps);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int DpsInc
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MeleeDpsInc);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int RangedDpsInc
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.RangedDpsInc);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int HDpsInc
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.HealDpsInc);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["spi"] = Spi;
                var res = formula.Evaluate(parameters);
        ...
        public override float MeleeAccuracy
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MeleeAntiMiss);
                var parameters = new Dictionary<string, double>();
        ...
        public override float MeleeCritical
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MeleeCritical);
                var parameters = new Dictionary<string, double>();
                parameters["str"] = Str; //Str not needed, but maybe we use later
                parameters["dex"] = Dex;
                var res = formula.Evaluate(parameters);
        ...
        public override float RangedAccuracy
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.RangedAntiMiss);
                var parameters = new Dictionary<string, double>();
        ...
        public override float RangedCritical
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.RangedCritical);
                var parameters = new Dictionary<string, double>();
                parameters["dex"] = Dex; //Str not needed, but maybe we use later
                parameters["int"] = Int;
                var res = formula.Evaluate(parameters);
        ...
        public override float SpellAccuracy
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.SpellAntiMiss);
                var parameters = new Dictionary<string, double>();
                parameters["int"] = Int;
                parameters["spi"] = Spi;
                var res = formula.Evaluate(parameters);
        ...
        public override float SpellCritical
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.SpellCritical);
                var parameters = new Dictionary<string, double>();
        ...
        public override float HealCritical
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.HealCritical);
                var parameters = new Dictionary<string, double>();
        ...
        public override int MagicResistance
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MagicResist);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                parameters["str"] = Str;
                parameters["dex"] = Dex;
        ...
        public override int Facets
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.Facet);
                var parameters = new Dictionary<string, double>();
                parameters["level"] = Level;
                var res = formula.Evaluate(parameters);
        ...
        public override float DodgeRate
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.Dodge);
                var parameters = new Dictionary<string, double>();
                parameters["dex"] = Dex;
                parameters["int"] = Int;
                var res = formula.Evaluate(parameters);
        ...
        public override float MeleeParryRate
        {
            ...
            {
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.MeleeParry);
                var parameters = new Dictionary<string, double>();
                parameters["str"] = Str;
                parameters["sta"] = Sta;
                var res = formula.Evaluate(parameters);
        ...
        public override float BlockRate
        {
            get
            {
                ...
                if (offhand != null && offhand.Template is WeaponTemplate template)
                {
                    ...
                    return 0f;
                var formula =
                    FormulaManager.Instance.GetUnitFormula(FormulaOwnerType.Mate, UnitFormulaKind.Block);
                var parameters = new Dictionary<string, double>();
                parameters["str"] = Str;
                var res = formula.Evaluate(parameters);
