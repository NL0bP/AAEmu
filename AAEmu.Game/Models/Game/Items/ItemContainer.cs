...
namespace AAEmu.Game.Models.Game.Items
{
    public class ItemContainer
    {
        ...
        public bool AcquireDefaultItemEx(ItemTaskType taskType, uint templateId, int amountToAdd, int gradeToAdd, out List<Item> newItemsList, out List<Item> updatedItemsList)
        {
            ...
            GetAllItemsByTemplate(templateId, (int)gradeToAdd, out var currentItems, out var currentTotalItemCount);
            ...
            if ((template.FixedGrade >= 0) && (template.Gradable == false))
                gradeToAdd = (int)template.FixedGrade;
            if (gradeToAdd == -1)
                gradeToAdd = (int)template.FixedGrade;
            if (gradeToAdd < 0)
                gradeToAdd = 0;
            ...
            while (amountToAdd > 0)
            {
                ...
                var newItem = ItemManager.Instance.Create(templateId, addAmount, (ItemGrade)gradeToAdd, true);
                amountToAdd -= addAmount;
                var prefSlot = -1;
        ...
        public int SpaceLeftForItem(Item itemToAdd, out List<Item> currentItems)
        {
            ...
            GetAllItemsByTemplate(itemToAdd.TemplateId, (int)itemToAdd.Grade, out currentItems, out var currentTotalItemCount);
        ...
        public bool GetAllItemsByTemplate(uint templateId, int gradeToFind, out List<Item> foundItems, out int unitsOfItemFound)
        {
            ...
            unitsOfItemFound = 0;
            foreach (var i in Items)
                if ((i.TemplateId == templateId) && ((gradeToFind < 0) || (gradeToFind == (int)i.Grade)))
                {
                    foundItems.Add(i);
