...
public class LootingContainer(IBaseUnit owner)
{
    ...
    public void OpenBag(Character player, BaseUnit object2, bool lootAll, float dist, bool autoLoot)
    {
        OpenedBy.Add(player);
        ...
        if (Items.Count > 0)
        {
            ...
            SendPacketToPlayers(OpenedBy, new SCLootingBagPacket(remainingItems, lootAll, autoLoot));
        }
    }
    ...
    public bool TryTakeLoot(Character player, ushort itemIndex, LootingContainerItemEntry itemEntry, bool didLootAll)
    {
        ...
        if ((itemEntry.HighestRoller > 0) && (itemEntry.HighestRoller != player.Id))
        {
            ...
            player.SendPacket(new SCLootItemFailedPacket(ErrorMessageType.NoPermissionToLoot, LootOwnerType, LootOwner.ObjId, itemEntry.ItemIndex, itemEntry.Item.TemplateId, player.ObjId));
            return false;
        }
        ...
        if (itemEntry.Item.Template.LootQuestId > 0)
        {
            ...
            {
                player.SendPacket(new SCLootItemFailedPacket(ErrorMessageType.NeedQuestToInteract, LootOwnerType, LootOwner.ObjId, itemEntry.ItemIndex, itemEntry.Item.TemplateId, player.ObjId));
                return false;
            }
        }
