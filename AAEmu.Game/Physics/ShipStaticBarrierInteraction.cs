...
public sealed class ShipStaticBarrierInteraction
{
    ...
    public void ResolveAll(InstanceWorld world, IReadOnlyList<Slave> ships, TimeSpan deltaTime)
    {
        if (ships.Count == 0)
            return;
    ...
    private static bool TryResolveShipVsSegment(InstanceWorld world, Slave ship, ShipStaticBarrier barrier, (float x0, float y0, float x1, float y1) seg, ref float separationBudgetMeters)
    {
