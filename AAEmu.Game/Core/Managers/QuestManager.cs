                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while (reader.Read())
                        {
                            ...
                            //template.HighlightDoodadPhase = reader.GetInt32("highlight_doodad_phase", -1); // TODO phase = 0?  // отсутствует в 0.5.101.406
                ...
                using(var command = connection.CreateCommand())
                {
                    ...
                    using(var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while(reader.Read())
                        {
                            ...
                            template.UseAlias = reader.GetBoolean("use_alias", true);
                            template.QuestActObjAliasId = reader.GetUInt32("quest_act_obj_alias_id", 0);
                            //template.HighlightDoodadId = reader.GetUInt32("highlight_doodad_id", 0); // отсутствует в 0.5.101.406
                            //template.HighlightDoodadPhase = reader.GetInt32("highlight_doodad_phase", -1); // TODO phase = 0? // отсутствует в 0.5.101.406
                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = new SQLiteWrapperReader(command.ExecuteReader()))
                    {
                        while (reader.Read())
                        {
                            ...
                            //template.LvlMinNpc = reader.GetInt32("lv_min_npc"); // отсутствует в 0.5.101.406
                            //template.LvlMaxNpc = reader.GetInt32("lv_max_npc"); // отсутствует в 0.5.101.406
                            //template.PcFactionId = reader.GetUInt32("pc_faction_id", 0); // отсутствует в 0.5.101.406
                            //template.PcFactionExclusive = reader.GetBoolean("pc_faction_exclusive", true); // отсутствует в 0.5.101.406
                            //template.NpcFactionId = reader.GetUInt32("npc_faction_id", 0); // отсутствует в 0.5.101.406
                            //template.NpcFactionExclusive = reader.GetBoolean("npc_faction_exclusive", true); // отсутствует в 0.5.101.406
