        ...
        public override PacketStream Write(PacketStream stream)
        {
            ...
            if (_unit is Character)
            {
                ...
                for (var i = 0; i < character.Inventory.Equipment.GetSlottedItemsList().Count; i++)
                {
                    ...
                    {
                        bodyPart.Write(stream);

                        //if (i != 19 && i != 20 && i != 23 && i != 24) { continue; }
            ...
            if (_unit is Character)
            {
                ...
                {
                    stream.Write(buff.Template.Id); // buff.Id
                    stream.Write(buff.Template.Level);
                }
            }
            else
            {
            ...
            if (_unit is Character)
            {
                stream.WritePisc(0, 0, ((Character)_unit).Appellations.ActiveAppellation, 0); // pisc 4
            }
            else
            {
            ...
            if (_unit is Character)
            {
                ...
                {
                    _unit.Effects.AddEffect(new Effect(_unit, _unit, SkillCaster.GetByType(SkillCasterType.Unit), SkillManager.Instance.GetBuffTemplate(8000011), null, System.DateTime.UtcNow));
                }

                if (!_unit.Effects.CheckBuff(8000012)) //TODO Wrong place
                {
                    _unit.Effects.AddEffect(new Effect(_unit, _unit, SkillCaster.GetByType(SkillCasterType.Unit), SkillManager.Instance.GetBuffTemplate(8000012), null, System.DateTime.UtcNow));
                }
            }
            ...
            foreach (var effect in goodBuffs)
            {
                ...
                stream.Write((byte)1);                 // sourceAbLevel
