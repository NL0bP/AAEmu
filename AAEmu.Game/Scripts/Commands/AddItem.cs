    ...
    public class AddItem : ICommand
    {
        ...
        public void Execute(Character character, string[] args)
        {
            ...
            if ((args.Length > firstarg + 2) && (byte.TryParse(args[firstarg + 2], out byte arggrade)))
                grade = (ItemGrade)arggrade;

            if (grade > ItemGrade.Mythic || grade < ItemGrade.Crude)
            {
                character.SendMessage("|cFFFF0000Item grade cannot be lower than {0} or exceed {1}!|r", ItemGrade.Crude, ItemGrade.Mythic);
                return;
            }
            ...
            if (itemTemplate.Category_Id == 133) // Speciality Packs (tradepacks) 
            {
                ...
                if (!targetPlayer.Inventory.Equipment.AcquireDefaultItem(ItemTaskType.Gm, itemId, count, (int)grade))
                {
            ...
            if (!targetPlayer.Inventory.Bag.AcquireDefaultItem(ItemTaskType.Gm, itemId, count, (int)grade))
            {
